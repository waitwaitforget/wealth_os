...
def test_cash_is_first_class_and_weights_sum_to_one():
    ...
    base = pd.Series({"EQ": .6, "BTC": .02, "CASH": .38})
    constraints = PortfolioConstraints(max_weights={"BTC": .03}, min_weights={"CASH": .05}, max_turnover=1.0)
    ...
    weights = policy.generate_target_weights(pd.Timestamp("2024-01-01"), base, signals, pd.Series({"EQ": .2, "BTC": .7, "CASH": 0}))
    assert abs(weights.sum() - 1) < 1e-9
    assert weights["CASH"] >= .05
    assert weights["BTC"] <= .03
