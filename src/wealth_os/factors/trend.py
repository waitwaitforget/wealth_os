...
class TrendFactor:
    ...
    def compute(self, prices: pd.DataFrame) -> pd.DataFrame:
        ...
        for period, weight in zip(self.periods, self.weights, strict=True):
            ...
            score = score.add(momentum.div(scale.replace(0, np.nan)).clip(-3, 3).mul(weight), fill_value=0)
        ma = prices.rolling(self.moving_average_window, min_periods=self.moving_average_window // 2).mean()
