...
def validate_prices(prices: pd.DataFrame) -> list[ValidationIssue]:
    ...
    if not prices.index.is_monotonic_increasing:
        issues.append(
            ValidationIssue("error", "time_order", "Price index is not monotonic increasing")
        )
    ...
    if prices.isna().mean().max() > 0.10:
        issues.append(
            ValidationIssue(
                "warning", "missing_data", "At least one asset has over 10% missing observations"
            )
        )
    return issues
...
def validate_accounting(result: BacktestResult, tolerance: float = 1e-6) -> list[ValidationIssue]:
    ...
    if float(error) > tolerance:
        issues.append(
            ValidationIssue("error", "nav_identity", f"NAV identity failed; max error={error}")
        )
    ...
    if float(unit_error) > tolerance:
        issues.append(
            ValidationIssue(
                "error", "unit_nav_identity", f"Unit NAV identity failed; max error={unit_error}"
            )
        )
...
def validate_weights(
    ...
    if (weights < -tolerance).any().any() and not constraints.allow_leverage:
        issues.append(
            ValidationIssue(
                "error", "negative_weight", "Negative weights found in long-only portfolio"
            )
        )
    for symbol, upper in constraints.max_weights.items():
        if symbol in weights and (weights[symbol] > upper + tolerance).any():
            issues.append(
                ValidationIssue("error", "max_weight", f"{symbol} exceeded max weight {upper}")
            )
    return issues
    ...
    if float(diff) > tolerance:
        return [
            ValidationIssue(
                "error",
                "lookahead",
                f"Historical factor values changed after future-data corruption; max diff={diff}",
            )
        ]
    return []
